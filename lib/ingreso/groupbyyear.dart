...
class Groupbyyear extends StatefulWidget {
  Groupbyyear({Key key}) : super(key: key);

  @override
  _GroupbyyearState createState() => _GroupbyyearState();
}

GlobalKey<ScaffoldState> _scalffoldkey = GlobalKey();

class _GroupbyyearState extends State<Groupbyyear> {
  ...
  _selectYear(
    ...
    DateTime _yearT = _year ?? DateTime(DateTime.now().year);
        ...
        builder: (BuildContext context) => Container(
              ...
              child: YearPicker(
                ...
                onChanged: (date) {
                  setState(() {
                    _year = date;
                    _inputFieldDateC.text =
                        DateFormat('yyyy', 'es').format(date);
                  });
  ...
  Widget _getBody() {
    return StreamBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot<List<Ingreso>> snapshot) {
          if (snapshot.hasData) {
            _list = snapshot.data;
            element = groupBy(_list, (obj) => DateTime(obj.fecha.year));
          } else {
            return Center(
    ...
    return Padding(
      ...
      child: Form(
        ...
        child: TextFormField(
          ...
          onSaved: (value) {
            if (_year != null) {
              ingresoProvider.searchIngresosbyYear(_year);
            }
          },
        ),
      ),
  ...
  bool compareYear(DateTime date1, DateTime date2) {
